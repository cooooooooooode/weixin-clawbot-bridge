...
function readChannelVersion(): string {
  try {
    ...
    const data = JSON.parse(fs.readFileSync(pkg, "utf-8")) as { version?: string }
...
async function apiFetch(params: {
  ...
  try {
    ...
    logger.debug(`${params.label} status=${res.status} raw=${redactBody(raw)}`)
    if (!res.ok) {
      throw new Error(`${params.label} ${res.status}: ${raw}`)
    }
    return raw
  } catch (err) {
    clearTimeout(t)
...
/** Send a single message downstream. Returns raw response body. */
